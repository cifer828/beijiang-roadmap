...
面向 2026 年 9 月 29 日至 10 月 9 日北疆自驾行程的手机 Web App。应用包含 11 天每日详情、22 个景点、住宿订单、精确高德地点跳转、全景路线、行前清单和双家庭固定 50/50 记账。
...
- 初始账本包含当前已知的 9 笔住宿（不含阿勒泰壹号，且已移除不入住的花间营）和 1 笔租车费用，共 ¥11,095.43，付款人均为张秋晨；两家固定 50/50 后，当前最简平账为家庭 A 向家庭 B 转账 ¥5,547.71。

本地直接运行 `npm run dev` 时不连接 Netlify，清单和消费保存在 `localStorage`。如需连同 Functions 本地联调，可使用已登录并已关联项目的 Netlify CLI 运行 `netlify dev`。

## 高德地点与地图

所有地点按钮都传入已核对的高德坐标和地点名，只在高德中展示标注点，不预设驾车、步行、骑行或自动导航。手机端直接调用高德官方地图标注 Scheme（iOS `iosamap://viewMap`、Android `androidamap://viewMap`），桌面端回落到 `uri.amap.com/marker`；当天入口打开当天终点，地图信息卡打开当前选中的标记点。阿禾公路、喀拉峻草原这类没有唯一且已确认落点的泛路线不展示单点按钮，避免把用户带到错误或非开放位置。
...
- 地图实例在 Tab 间保持复用；脚本或渲染失败时显示地点清单和“重新加载地图”，不会留下空白页。

原始迁移图片和用户提供的订单截图均不修改；景点封面使用复制到 `public/images/` 的版本，住宿卡已停止展示酒店外观素材，改为订单摘要和可点开的订单截图。订单截图由共享账本作为独立凭证 Blob 保存。

租车已改为门店自行取还：9 月 30 日 09:00 前往乌鲁木齐经开区高铁北二路 100 号凯坤壹品 1# 底商住宅楼商业 103 室取车，10 月 9 日返回同一门店还车。还车时间尚未确认，应用会持续显示重要提醒，避免沿用已失效的 14:00 机场上门还车安排。
