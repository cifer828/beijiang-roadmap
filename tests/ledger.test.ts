...
describe("双家庭固定 50/50 结算", () => {
  it("除阿勒泰一号外的住宿与租车都记录为张秋晨支付", () => {
    expect(INITIAL_EXPENSES).toHaveLength(10);
  ...
  it("当前已知订单只生成一笔跨家庭转账", () => {
    ...
    expect(result.total).toBe(1109543);
    expect(result.paidA).toBe(0);
    expect(result.paidB).toBe(1109543);
    expect(result.owedA).toBe(554771);
    expect(result.owedB).toBe(554772);
    expect(result.transfer).toEqual({ from: "闫寒 · 刘一帆", to: "张秋晨 · 王晶", amountCents: 554771 });
  });
